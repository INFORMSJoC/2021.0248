function [x, out] = PSGs_std_x(f_g_handle,opts)
% PSGs_std_x is a code written by Xiantao Xiao
% for solving SSD constrained portfolio optimization problem
% with PSG proposed in the following paper: 
...
% step size
alpha0 = opts.alpha0; beta0 = opts.beta0; gamma0 = opts.gamma0;
if ssize_a
a = 3/4; b = 0; c = 1/2;
else
a = 7/8; b = 1/2; c = 3/4;
...
%             cons            = max(con_I);  
        ...
        % step size set up
        alpha           = max(alpha0 / (  iter^a),step_min);
        beta            = max(beta0 / (iter^b),step_min);
        gamma           = max(gamma0 / ( iter^c),step_min);
...
%              for j = 1:ssize_I   
%                 ind             = sample_Ind(j);  
%                 g_s             = f_g_handle.get_s_g(x,ind);
%                 t(ind)          = (1 - beta) * t(ind) + beta * g_s; 
%              end            
%         else    
             for j = 1:N   
                ind             = j;
